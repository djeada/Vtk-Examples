...
FILE_NAME = os.path.join(SCRIPT_DIR, "../../data/vtps/naca0012.vtp")
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "../../data/vtps/naca0012_transformed.vtp")


def write_vtp(data: vtk.vtkPolyData, filename: str):
    """Write a vtkPolyData to a VTP (XML PolyData) file."""
...
def read_vtp(filename: str) -> vtk.vtkPolyData:
    """Read a VTP (XML PolyData) file and return a vtkPolyData."""
...
if __name__ == "__main__":
    ...
    # Read VTP file
    vtp_data = read_vtp(FILE_NAME)

    # Print dataset info
    print_dataset_info(vtp_data)

    # Display for verification
    mapper = vtk.vtkPolyDataMapper()
    mapper.SetInputData(vtp_data)
    ...
    vtp_data = transform_polydata(vtp_data, transform)

    # Write VTP file
    write_vtp(vtp_data, OUTPUT_FILE)
