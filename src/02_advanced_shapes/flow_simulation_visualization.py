...
def compute_pressure_coefficient(
    u: np.ndarray, v: np.ndarray, u_inf: float = U_INF
) -> np.ndarray:
    """
...
def compute_pressure(
    cp: np.ndarray, rho: float = RHO, u_inf: float = U_INF, p_inf: float = P_INF
) -> np.ndarray:
    """
...
def create_cylinder_geometry(
    radius: float = CYLINDER_RADIUS, num_points: int = 100
) -> vtk.vtkPolyData:
    """
...
def create_flow_field_grid(
    resolution: int = GRID_RESOLUTION, domain_size: float = DOMAIN_SIZE
...
def add_velocity_to_grid(
    grid: vtk.vtkStructuredGrid, u: np.ndarray, v: np.ndarray
) -> None:
    """
...
def add_scalar_to_grid(
    grid: vtk.vtkStructuredGrid, scalar: np.ndarray, name: str
) -> None:
    """
...
def create_streamline_actor(
    grid: vtk.vtkStructuredGrid, domain_size: float = DOMAIN_SIZE
) -> vtk.vtkActor:
    """
...
def create_pressure_contour_actor(
    grid: vtk.vtkStructuredGrid, x: np.ndarray, y: np.ndarray, cp: np.ndarray
) -> vtk.vtkActor:
    """
...
def create_scalar_bar(
    lut: vtk.vtkLookupTable, title: str = "Pressure Coefficient (Cp)"
...
def run_flow_simulation() -> None:
    ...
    print(
        f"  Pressure coefficient range: [{np.min(cp_valid):.3f}, {np.max(cp_valid):.3f}]"
    )
    print(
        f"  Velocity magnitude range:   [{np.min(vel_valid):.3f}, {np.max(vel_valid):.3f}] m/s"
    )
    print()
